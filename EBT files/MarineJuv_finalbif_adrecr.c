...
Last modification: AvL - 29 Apr 2025
...
--29 Apr 25: Include Invasion for bifurcation runs (response to reviews ICES JMS) 
...
#define inv                             parameter[28] // additional recruitment from outside / open system
...
/*// 20250507 moving invasion code here, so it influences the inflow into new cohorts
#if ((INVADE == 1) && (BIFURCATION == 1))
      double            invdummy = fmod(time, BifPeriod);
      if (((invdummy + 0.5*cohort_limit) > (BifPeriod - 2*INVADEPERIOD*SEASON)) &&
          ((invdummy + 0.5*cohort_limit) < (BifPeriod -   INVADEPERIOD*SEASON)))
        {
          newborns = INVADEDENSITY;
...
//              memset((void *)ReproOutputs, 0, (INVADEPERIOD*INVADEPERIOD)*sizeof(double));
...
#endif */  // changing the invasion code again to more properly account for recruitment from open system
    ...
    #if (INVADE == 1)
      //printf("At time %f newcohort is: %d and %d\n", time, newcoh, newcoh + 1);
      pop[0][newcoh][number] = popIDcard[0][newcoh][IDnbegin] = connect * egg_frac * newborns + (inv);
    #endif
...
// 20250507 moving invasion code upwards, so it influences the inflow into new cohorts    
/*#if ((INVADE == 1) && (BIFURCATION == 1))
      double            invdummy = fmod(time, BifPeriod);
      if (((invdummy + 0.5*cohort_limit) > (BifPeriod - 2*INVADEPERIOD*SEASON)) &&
          ((invdummy + 0.5*cohort_limit) < (BifPeriod -   INVADEPERIOD*SEASON)))
